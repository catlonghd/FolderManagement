...
void BIOS_Parameter_Block::readSector(BYTE*& sector) {
	BPB_BytsPerSec = hexToDec(sector, 0x0B, 2);
	BPB_SecPerClus = hexToDec(sector, 0x0D, 1);
	...
	//lấy version FAT32
	char mijor = hexToDec(sector, 0x2A, 1);
	char minor = hexToDec(sector, 0x2B, 1);
	BPB_FSVer = mijor + "." + minor;
...
string BIOS_Parameter_Block::output() {
	stringstream ss;
	ss << "- Boot sector: \n";
	...
	ss << "\t Type of volume: " << BPB_Media << endl;
	...
	ss << "\t Backup sector contains BIOS_Parameter_Block: " << BPB_BkBootSec << endl;
	ss << "\t Reversed: " << BPB_Reversed << endl;
	return ss.str();
}


BIOS_Parameter_Block::BIOS_Parameter_Block(){}

BIOS_Parameter_Block::BIOS_Parameter_Block(BYTE*& sector) {
	readSector(sector);
}
